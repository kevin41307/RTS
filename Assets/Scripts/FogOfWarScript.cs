...
public class FogOfWarScript : MonoBehaviour
{
	...
	public float powStrengh = 2f;
	public float clampStrengh = 0.05f;
	...
	void Update()
	{
		...
		if (Physics.Raycast(r, out hit, 1000, m_fogLayer, QueryTriggerInteraction.Collide))
		{
			for (int i = 0; i < m_vertices.Length; i++)
			{
				...
				if (dist < m_radiusSqr)
				{
					float alpha = Mathf.Min(m_colors[i].a, Mathf.Clamp01( Mathf.SmoothStep(clampStrengh, 1, Mathf.Pow(dist / m_radiusSqr, powStrengh))));
					m_colors[i].a = alpha;
				}
			}
			UpdateColor();
	...
	void Initialize()
	{
		...
		//Debug.Log(i + " " + m_vertices[i].x + " " + m_vertices[i].y + " " + m_vertices[i].z);
